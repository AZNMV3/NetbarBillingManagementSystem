...
void purview_man_add_admin(void)
{
	while (true) {
		...
		if (!scanf("%d", &is_super)) {
			printf("\n输入非法！退出");
			MessageBoxA(NULL, "输入非法！", "权限管理", MB_OK);
			fflush(stdin);
			continue;
		}
		...
		if (is_super) {
			...
			while (temp != 'f') {
				scanf("%c", &temp);
				switch (temp) {
					case 'a': allow_cardman = 1;
						break;
					case 'b': allow_billman = 1;
						break;
					case 'm': allow_shutman = 1;
						break;
					case 'c': allow_chargeman = 1;
						break;
					case 's': allow_statman = 1;
						break;
					case 'f': break;
					default
					: break;
