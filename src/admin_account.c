...
bool admin_is_passwd_right(char user[], char password[])
{
	FILE* fp = fopen(FILE_POSITION, "r");
	fseek(fp, 0, SEEK_END);
	long len = ftell(fp);
	...
	encrypt_password(password);
	cJSON* data_json = cJSON_GetObjectItem(root_json, user);
...
bool admin_has_user(char user[])
{
	...
	if (cJSON_HasObjectItem(root_json, user))
		return true;
...
void admin_set_permission(char* name, char type, int new_value)
{
	...
	cJSON* data_json = cJSON_GetObjectItem(root_json, name);
	switch (type) {
		case 'a': cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_cardman"), new_value);
			break;
		case 'b': cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_billman"), new_value);
			break;
		case 'm': cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_shutman"), new_value);
			break;
		case 'c': cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_chargeman"), new_value);
			break;
		case 's': cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_statman"), new_value);
			break;
		case 1: {
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "su"), new_value);
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_cardman"), new_value);
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_billman"), new_value);
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_shutman"), new_value);
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_chargeman"), new_value);
			cJSON_SetNumberValue(cJSON_GetObjectItem(data_json, "allow_statman"), new_value);
		}
			break;
		default: return;
	}
	fp = fopen(FILE_POSITION, "wb+");
	fwrite(cJSON_Print(root_json), strlen(cJSON_Print(root_json)), 1, fp);
	fclose(fp);
	free(data);
	cJSON_Delete(root_json);
}

struct admin _write_permissions(char user[])
{
	struct admin result;
